#!/usr/bin/env python3
"""
AgroTechAI Sensor-Only WebSocket Performance Benchmark

Este script mide las métricas de rendimiento del sistema AgroTechAI
para análisis de solo sensores (sin imagen):
- Latencia total de análisis (p50, p99, avg, min, max)
- Latencia por agente (AgriVision, SoilSense, CropMaster)
- Throughput (análisis por minuto)

Uso:
    python benchmark_sensor.py --requests 10
    python benchmark_sensor.py --requests 20 --host localhost --port 5000
"""
...
async def run_single_analysis(
    ...
    """
    Ejecuta un análisis de sensores y mide tiempos por agente.
    ...
    try:
        async with websockets.connect(uri, ping_interval=None) as ws:
            ...
            if verbose:
                print(f"  [Request {request_id}] Enviado análisis de sensores...")
            ...
            while True:
                try:
                    ...
                    if msg_type == "status":
                        ...
                        if "AgriVision" in status_msg or "SoilSense" in status_msg:
...
async def run_benchmark(
    ...
    print(f"\n{'='*60}")
    print(f"AgroTechAI Sensor-Only Benchmark ({mode})")
    print(f"{'='*60}")
    print(f"Endpoint: {uri}")
    print(f"Requests: {num_requests}")
    if concurrent > 1:
        print(f"Concurrencia: {concurrent} requests simultáneos")
    print(f"Agentes: AgriVision, SoilSense, CropMaster (sin ImageVision)")
    print(f"{'='*60}\n")

    # Verificar conexión
    print(f"Verificando conexión a {uri}...")
    try:
        async with websockets.connect(uri, ping_interval=None) as ws:
    ...
    if concurrent > 1:
        ...
        result = await run_concurrent_benchmark(
            uri, crop_description, environment_description, num_requests, concurrent, verbose
        )
    else:
        # Modo secuencial
...
def print_results(result: BenchmarkResult, ctq_latency_ms: float = 180000):
    """Imprime los resultados del benchmark"""

    print(f"\n{'='*60}")
    print(f"RESULTADOS DEL BENCHMARK (Sensor-Only)")
    print(f"{'='*60}\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="AgroTechAI Sensor-Only WebSocket Performance Benchmark",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Ejemplos:
  python benchmark_sensor.py --requests 10
  python benchmark_sensor.py --requests 20 --verbose
  python benchmark_sensor.py --requests 5 --host 192.168.1.100 --port 8000
        """
    )
    ...
    parser.add_argument(
        "--crop", "-c",
        default="Cultivo de tomate en estado vegetativo, hojas verdes con algunas manchas amarillas, posible deficiencia de nutrientes",
        help="Descripción del cultivo para el análisis"
    )

    parser.add_argument(
        "--environment", "-e",
        default="Humedad del suelo 65%, Temperatura 23°C, pH 6.7, condiciones normales de campo abierto",
    ...
    parser.add_argument(
        ...
        type=float,
        default=180000,
        help="Objetivo CTQ de latencia p99 en ms (default: 180000)"
    )
